export type ClassId = 'mage' | 'warrior' | 'paladin';
...
export interface AbilityDef { name: string; description: string; cost: number; cooldown: number; range: number; damage: number; kind: 'projectile' | 'melee' | 'area' | 'heal' | 'shield' | 'dash'; radius: number; color: string; duration?: number; speed?: number; }
