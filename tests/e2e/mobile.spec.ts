...
test('mobile notices stay silent and errors replace each other without a queue', async ({ browser }) => {
  const context = await browser.newContext({ viewport: { width: 568, height: 320 }, isMobile: true, hasTouch: true });
  const page = await context.newPage();
  let notice: (message: string, tone: string) => void = () => { throw new Error('Socket not connected'); };
  await page.routeWebSocket('**/ws', socket => {
    socket.connectToServer();
    notice = (message, tone) => socket.send(JSON.stringify({ type: 'notice', message, tone }));
  });
  await page.addInitScript(() => Object.defineProperty(navigator, 'standalone', { value: true }));
  try {
    await register(page, 'Notices');
    notice('Fuori dall’avamposto: PvP attivo.', 'info');
    notice('Zona sicura: PvP disattivato.', 'info');
    notice('+10 gold raccolti', 'success');
    notice('Errore di prova', 'error');
    await expect(page.locator('.toast')).toHaveCount(1);
    await expect(page.locator('.toast')).toHaveText('Errore di prova');
    notice('Nuovo errore di prova', 'error');
    await expect(page.locator('.toast')).toHaveCount(1);
    await expect(page.locator('.toast')).toHaveText('Nuovo errore di prova');
    await expect(page.locator('.toast')).toHaveCount(0, { timeout: 10000 });
  } finally { await context.close(); }
});

test('iOS fullscreen fallback stays silent and standalone skips fullscreen', async ({ browser }) => {
