...
test('dungeon authoring persists and exports; touch movement remains responsive with 30 ms transport RTT', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    const draft = () => page.evaluate(() => JSON.parse(localStorage.getItem('riftlands.dungeon-draft.v2')!) as DungeonDraft);
    ...
    const beforePlacement = await draft();
    await page.locator('#world-placement').click();
    const picker = page.locator('.world-picker'), worldMap = picker.locator('canvas');
    await expect(picker).toBeVisible();
    const worldBox = (await worldMap.boundingBox())!;
    await worldMap.click({position:{x:worldBox.width/2,y:worldBox.height/2+3200*worldBox.width/16000}});
    await expect(picker.locator('[data-apply]')).toBeDisabled();
    await expect(picker.locator('.placement-status')).toContainText('avamposto');
    await worldMap.click({position:{x:worldBox.width*.8,y:worldBox.height*.2}});
    await expect(picker.locator('[data-apply]')).toBeEnabled();
    await page.screenshot({path:'test-results/dungeon-world-placement.png'});
    await picker.locator('[data-apply]').click();
    assert.notDeepEqual((await draft()).origin,beforePlacement.origin);
    assert.deepEqual((await draft()).entities,beforePlacement.entities);
    await page.locator('#undo').click();assert.deepEqual((await draft()).origin,beforePlacement.origin);
    await page.locator('#redo').click();assert.notDeepEqual((await draft()).origin,beforePlacement.origin);
    await page.locator('#file').setInputFiles({name:'runtime.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(multi))});
    await expect(page.locator('#status')).toContainText('runtime riconosciuta');
    assert.equal((await draft()).entities.filter(e=>e.kind==='boss').length,2);
    await page.screenshot({ path: 'test-results/dungeon-maker-desktop.png', fullPage: true });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#world-placement').click();
    await expect(picker).toBeVisible();
    assert.equal(await picker.evaluate(e=>e.scrollWidth<=e.clientWidth),true);
